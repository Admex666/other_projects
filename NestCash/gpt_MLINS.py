...
if __name__ == "__main__":
    ...
    if 'df' in locals():
        ...
        feats = pd.DataFrame({
            ...
            'vel7':   [df_velocity[df_velocity['user_id']==user_id]['velocity'].iloc[-1]],
            'diversity': [div],
            'fixed_ratio': [fv]
        })
        ...
        print(f"\n👤 Felhasználó: {user_id}")
        print("────────────────────────────────────────")
        print(f"  • Kockázat: {risk_msg}")
        print(f"  • Átlagos költés (7/30/90 nap): {roll7:.0f} / {roll30:.0f} / {roll90:.0f} HUF")
        print(f"  • Kategória-diverzitás: {div:.3f} (átlag: {div_bm:.3f})")
        print(f"  • Megtakarítás változás az utolsó előtti hónapban: {trend_ch:+.1%}pont")
        print(f"  • Fix költségeid aránya: {fv:.0%} (átlag: {fvr_bm:.0%})")
        print(f"  • Modellünk szerint ezekkel mutatókkal kb. {suggested_assets:,.0f} HUF vagyont érhetsz el. (jelenlegi: {assets_now:,.0f} HUF)")
        print("────────────────────────────────────────")
