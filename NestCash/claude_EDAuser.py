...
class UserFinancialEDA:
    ...
    def analyze_user(self, user_id, show_plots=True):
        ...
        if show_plots:
            self._create_user_dashboard(user_data, profile_data, user_profile)
    ...
    def _create_user_dashboard(self, user_data, profile_data, user_profile):
        ...
        fig.suptitle(f'Személyes Pénzügyi Dashboard - {user_profile} profil', fontsize=16, fontweight='bold')
    ...
    def analyze_multiple_users(self, user_ids, show_plots=True):
        """Több user elemzése egyszerre"""
        results = {}
        
        for user_id in user_ids:
            print(f"\n{'='*100}")
            result = self.analyze_user(user_id, show_plots=show_plots)
            if result:
...
# Használat
def run_user_eda(df, user_ids=None, sample_size=3):
    """
    User-specifikus EDA futtatása
    
    Parameters:
    df: DataFrame a tranzakciókkal
    user_ids: Lista a vizsgálandó user ID-kről (None esetén random mintát vesz)
    sample_size: Ha user_ids None, ennyi random usert elemez
    """
    ...
    # User ID-k meghatározása
    if user_ids is None:
        all_users = eda.get_all_users()
        if len(all_users) > sample_size:
            user_ids = np.random.choice(all_users, size=sample_size, replace=False)
        else:
            user_ids = all_users
        print(f"\n🔍 Véletlenszerűen kiválasztva {len(user_ids)} felhasználó:")
        print(", ".join(map(str, user_ids)))
    else:
        print(f"\n🔍 Megadott felhasználók: {len(user_ids)}")
    ...
    results = eda.analyze_multiple_users(user_ids)
    
    # Összegzés
    print("\n" + "="*100)
    print("✅ ELEMZÉS BEFEJEZVE")
    print("="*100)
    print(f"Összesen {len(results)} felhasználó elemzésre került")
    
    return results

#%% Példa használat:
df = pd.read_csv('szintetikus_tranzakciok.csv')
eredmenyek = run_user_eda(df, user_ids=[40])

#eredmenyek = run_user_eda(df, sample_size=5)