...
def generate_user_transactions(user, months=3):
    ...
    data = []
    balance = 0
    savings = sum(user["befektetesek"].values())
    ...
    for month_offset in range(months):
        ...
        for bef_tipus, alap in user.get("befektetesek", {}).items():
            ...
            hozam_szazalek = random.uniform(0.04, 0.10) / 12
        ...
        for _ in range(100):
            ...
            ttypes = ["vagy", "megtakaritas", 'impulzus']
            ttype_probs = [0.5, 0.3, 0.2]  
            ttype = random.choices(ttypes, weights=ttype_probs, k=1)[0]

            category = random.choice(CATEGORIES[ttype]["kategorialista"])
