...
def get_all_beta_users() -> List[Dict[str, Any]]:
    """Returns all beta users with activity statistics from Supabase (or SQLite only if not configured)."""
    if is_supabase_configured():
        ...
        if sb:
            try:
                ...
                print(f"[USER SERVICE ERROR] Supabase get_all_beta_users failed: {e}")
                return []
        return []
