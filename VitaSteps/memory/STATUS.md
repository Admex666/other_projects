...
*   **Nagy-Kevély Landing Page Megújítás (UPDATED 2026-07-13):**
    ...
    *   **Adatbázis Normalizáció & Több Kihívás Támogatása (COMPLETED 2026-07-16):**
        ...
        *   **Google Sheets Lecsatolás és Adatmodell Bővítés (COMPLETED 2026-07-17):** Megszüntettük a Google Sheets írásokat a fizetési folyamatban (`process-payment.js` és `stripe-webhook.js` refaktor). Létrehoztuk az új `orders` és `shipments` táblákat a Supabase-ben, kibővítettük a `runners` és `runs` mezőit, és az összes fizetési/szállítási adatot immár közvetlenül a Supabase-ben tároljuk. A korábbi adatok fizikai migrációját külön feladatként felvezettük a `memory/TASKS.md` listára.
        *   **Foxpost Tömeges Export & Szállítás Kezelés (COMPLETED 2026-07-20):** Integráltuk a Foxpost tömeges export felületet az `admin.html`-be. SheetJS segítségével a böngészőben közvetlenül generálható a hivatalos Foxpost import sablonnak megfelelő XLSX fájl. Ezzel egy időben megvalósítottuk a kijelölt csomagok csoportos feladottnak jelölését (Supabase update a `/api/admin-approve` segítségével).
